            ...
            <tr v-for="order in orders" :key="order.trade_no">
              ...
              <td>
                <div class="flex items-center gap-1.5">
                  ...
                  <span class="text-sm font-medium" :class="order.type === 1 ? 'text-blue-600' : 'text-green-600'">{{
                    order.type === 1 ? '支付宝' : '微信' }}</span>
                </div>
              </td>
              <td class="font-semibold text-emerald-600">¥{{ order.money }}</td>
              <td>
                <span v-if="order.status === 1"
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200">
                  <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5"></span>已支付
                </span>
                <span v-else
                  class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-700 ring-1 ring-amber-200">
